...
class TransformersDatasetWeakView(PytorchDatasetView):

    def clone(self):
        if self.is_multi_label:
            data = [(torch.clone(d[TransformersDatasetWeak.INDEX_TEXT]),
                     torch.clone(d[TransformersDatasetWeak.INDEX_MASK]),
                     d[TransformersDatasetWeak.INDEX_LABEL].copy(),
                     d[TransformersDatasetWeak.INDEX_WEAKLABEL].copy()) for d in self.data]
        else:
            data = [(torch.clone(d[TransformersDatasetWeak.INDEX_TEXT]),
                     torch.clone(d[TransformersDatasetWeak.INDEX_MASK]),
                     np.copy(d[TransformersDatasetWeak.INDEX_LABEL]),
                     np.copy(d[TransformersDatasetWeak.INDEX_WEAKLABEL])) for d in self.data]

        target_labels = None if self.dataset.track_target_labels else np.copy(self.target_labels)
        return TransformersDatasetWeak(data,
...
class TransformersDatasetWeak(PytorchDataset):
    ...
    def __init__(self, data, multi_label=False, target_labels=None):
        ...
        Parameters
        ----------
        data : list of 4-tuples (text data [Tensor], mask [Tensor], labels [int or list of int], weak label [int])
    ...
    def x(self, x):
        for i, _x in enumerate(x):
            self._data[i] = (_x, self._data[i][self.INDEX_MASK], self._data[i][self.INDEX_LABEL], self._data[i][self.INDEX_WEAKLABEL])
        ...
        if self.multi_label:
            ...
            for i, p in enumerate(range(num_samples)):
                self._data[i] = (self._data[i][self.INDEX_TEXT],
                                 ...
                                 self._data[i][self.INDEX_WEAKLABEL])
        else:
    ...
    def clone(self):
        if self.is_multi_label:
            data = [(torch.clone(d[self.INDEX_TEXT]),
                     ...
                     d[self.INDEX_WEAKLABEL].copy()) for d in self._data]
        else:
            data = [(torch.clone(d[self.INDEX_TEXT]),
                     torch.clone(d[self.INDEX_MASK]),
                     np.copy(d[self.INDEX_LABEL]),
                     np.copy(d[self.INDEX_WEAKLABEL])) for d in self._data]
    ...
    def to(self, other, non_blocking=False, copy=False):
        ...
        data = [(d[self.INDEX_TEXT].to(other, non_blocking=non_blocking, copy=copy),
                 ...
                 d[self.INDEX_LABEL],
                 d[self.INDEX_WEAKLABEL]) for d in self._data]

        if copy is True:
            target_labels = None if self.track_target_labels else self._target_labels
            return TransformersDatasetWeak(data, target_labels=target_labels)
        else:
            self._data = data
            return self
    ...
    @classmethod
    @experimental
    def from_arrays(cls, texts, y, yweak, tokenizer, target_labels=None, max_length=512):
        ...
        return TransformersDatasetWeak(data_out, multi_label=multi_label, target_labels=target_labels)

    def __getitem__(self, item):
        return TransformersDatasetWeakView(self, item)
